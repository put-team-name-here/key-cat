...
- **상점**: 씨앗(당근 5 · 양배추 15 · 토마토 50 · 복숭아 250 · 두리안 1,000코인), 고양이(샴 100,000 · 스핑크스 50,000 · 페르시안 500,000 · 러시안 블루 250,000 · 브리티시 쇼트헤어 250,000코인), 기능성 장난감 5종을 구매한다. 토마토는 치즈, 복숭아는 그레이, 두리안은 페르시안 고양이 보유 시 해금된다. 수량 선택 시 보유 코인 기준 최대 구매 가능 수량으로 채워주는 MAX 버튼 제공.
...
- **장난감 구매**: 가격은 75,000~225,000코인이며 종류별 1개만 보유할 수 있다. 구매한 장난감은 상점에서 `보유 중`으로 표시된다.
- **장난감 효과**: 집에 배치한 실뭉치·낚시대·인형·쥐 인형·스프링은 성장 속도, 작물 판매가, 100타 보너스 코인을 올린다. 서로 다른 장난감의 효과는 함께 적용된다.
- **기록**: 이번 달 날짜별 타자 수를 잔디 히트맵으로 보여주고, 연속 기록일과 이번 달 누적 타자 수를 표시. 날짜를 누르면 해당 일자 타자 수 상세를 확인할 수 있다.

**맵 이동/집 꾸미기**: 확장 화면은 집 → 농장 → 확장 농장 순서로 이동한다. 확장 농장은 처음에는 모두 잔디이며, 기존 밭과 같은 위치의 4×4 영역을 1번부터 16번까지 순서대로 구매한다(칸당 500,000코인, 전체 8,000,000코인). 구매한 칸은 기존 밭처럼 파종·자동 농사·수확에 사용한다. 큰 농장 화면의 돋보기 옆 완성 작물 아이콘을 누르면 자동 파종에서 먼저 사용할 씨앗을 정할 수 있다. 제공된 내비게이션 PNG와 목적지 아이콘을 축소해 사용하며, 하단 인디케이터로 현재 위치를 확인할 수 있다. 집 바닥은
